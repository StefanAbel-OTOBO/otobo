...
sub _NotificationFilter {
    ...
    # collect the to be ignored related attributes into a single hash
    my %AttributeIsIgnored;
    {
        ...
        for my $Key ( sort keys $IgnoredAttributes->%* ) {
            next KEY unless IsHashRefWithData( $IgnoredAttributes->{$Key} );

            @AttributeIsIgnored{ keys $IgnoredAttributes->{$Key}->%* }
                = values $IgnoredAttributes->{$Key}->%*;
        }
    }
    ...
    for my $Key ( sort keys $Notification{Data}->%* ) {
        ...
        next KEY if $AttributeIsIgnored{$Key};
