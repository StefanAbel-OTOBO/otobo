...
Core.UI.InputFields = (function (TargetNS) {
    ...
    */
    var Config = {
        ...
        SelectionBoxOffsetLeftCustomer: 24,
        SelectionBoxOffsetRight: 5,
        SelectionBoxOffsetRightCustomer: 8,
        ErrorClass: 'Error',
    ...
    TargetNS.Init = function () {
        Config.CustomerInterface = ( Core.Config.Get('SessionName') === Core.Config.Get('CustomerPanelSessionName') );
        ...
        if ( Config.CustomerInterface ) {
            ...
            $('fieldset > .Row, fieldset > .RichTextHolder').each( function() {
                ...
                TextInput.blur( function() {
                    // select boxes are handled in Core.UI.InputFields.js
    ...
    function ShowSelectionBoxes($SelectObj, $InputContainerObj) {
        ...
        if ( Config.CustomerInterface === true ) {
            $InputContainerObj.parent().removeClass('oooSelected');
            // for DynamicFields
            $InputContainerObj.parent().parent('.Field').removeClass('oooSelected');
        }
        ...
        if ($SelectObj.val()) {
            ...
            $.each(Selection, function (Index, Value) {
                ...
                if (OffsetLeft === 0) {
                    if ( Config.CustomerInterface === true ) {
                        OffsetLeft = Config.SelectionBoxOffsetLeftCustomer;
                        Config.SelectionBoxOffsetRight = Config.SelectionBoxOffsetRightCustomer;
                    }
                    else {
    ...
    function HideSelectList($SelectObj, $InputContainerObj, $SearchObj, $ListContainerObj, $TreeContainerObj) {
       ...
       if ($ListContainerObj && $TreeContainerObj) {
            ...
            if ( Config.CustomerInterface && $SelectObj.attr('id')) {
                var $LabelObj = $('label[for="' + Core.App.EscapeSelector($SelectObj.attr('id')) + '"]');
                if ($LabelObj.length > 0) {
                    $LabelObj.removeClass('oooFocus');
                }
            }
        }
                        ...
                        if (AvailableHeightTop > AvailableHeightBottom) {
                            ...
                            if ( Config.CustomerInterface === true ) {
                                $ListContainerObj
                                    ...
                                    .css({
                                        top: 'auto',
                                        bottom: parseInt($('body').height() - $InputContainerObj.offset().top - 8, 10)
                                    });
                            }
                            else {
                                $ListContainerObj
                            ...
                            if ( Config.CustomerInterface === true ) {
                                $ListContainerObj
                                    ...
                                    .addClass('ExpandToBottom')
                                    .css({
                                        top: parseInt($InputContainerObj.offset().top + 48, 10),
                                        bottom: 'auto'
                                    });
                            }
                            else {
